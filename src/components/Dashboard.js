  ...
  getTaxReturns: async () => {
    return new Promise((resolve) => {
      setTimeout(() => {
        ...
        const storedReturns = localStorage.getItem('taxReturns');
        const taxReturns = storedReturns ? JSON.parse(storedReturns) : [];
        resolve(taxReturns);
      }, 800);
    });
  },
  ...
  updateTaxReturn: async (id, data) => {
    return new Promise((resolve) => {
      setTimeout(() => {
        ...
        const storedReturns = localStorage.getItem('taxReturns');
        const taxReturns = storedReturns ? JSON.parse(storedReturns) : [];
        const updatedReturns = taxReturns.map(tr => tr.id === id ? updatedReturn : tr);
        localStorage.setItem('taxReturns', JSON.stringify(updatedReturns));
